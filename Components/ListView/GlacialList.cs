...
namespace GlacialComponents.Controls
{
	...
	public class GlacialList : System.Windows.Forms.Control
	{
		...
		{
			//this.Columns.
			if ( this.ControlStyle == GLControlStyles.XP )	Application.EnableVisualStyles();
        ...
        public string[][] —писки¬ыпадающихћеню;
        ...
        protected void ActivateEmbeddedControl(int nColumn, GLItem item, GLSubItem subItem, object[] obj)
		{
			if ( this.m_ActivatedEmbeddedControl != null )
			{
				this.m_ActivatedEmbeddedControl.Dispose();
			...
			icontrol.GLLoad( item, subItem, this, obj );
		...
		virtual public void DrawColumnHeader( Graphics graphicsColumn, Rectangle rectColumn, GLColumn column )
		{
            ...
            {
                if (rectColumn.Width == 0) rectColumn.Width = 10;
                if (rectColumn.Height == 0) rectColumn.Height = 10;
		...
		protected override void OnClick(EventArgs e)
		{
            ...
            string[] obj = null;
            if (—писки¬ыпадающихћеню != null && —писки¬ыпадающихћеню.Length > nItem) obj = —писки¬ыпадающихћеню[nItem];

			if ( ( listRegion == GLListRegion.client ) && ( nColumn < Columns.Count ) )
			{
                ActivateEmbeddedControl(nColumn, Items[nItem], Items[nItem].SubItems[nColumn], obj);
			}
