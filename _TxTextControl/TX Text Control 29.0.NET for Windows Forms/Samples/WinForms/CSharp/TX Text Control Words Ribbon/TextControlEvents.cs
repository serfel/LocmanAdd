	...
	public partial class MainWindow {
		...
		**-----------------------------------------------------------------------------------------------------*/
		private void TextControl_InputPositionChanged(object sender, EventArgs e) {
			...
			m_tableToolsGroup.Visible = m_textControl.Tables.GetItem() != null && !m_reportingPreviewGroup.Visible;
		}
		...
		**------------------------------------------------------------------------------------------------------*/
		private void TextControl_FrameSelected(object sender, TXTextControl.FrameEventArgs e) {
			...
			if (chartFrame != null) {
				m_frameToolsGroup.Header = Resources.CONT_TAB_GRP_CHART_TOOLS;	// Rename frame tools tab group temporarily
		...
		**------------------------------------------------------------------------------------------------------*/
		private void TextControl_FrameDeselected(object sender, TXTextControl.FrameEventArgs e) {
			...
			if ((e.Frame is ChartFrame) && !(frame is ChartFrame)) {
				m_frameToolsGroup.Header = Resources.CONT_TAB_GRP_FRAME_TOOLS;	// Restore old group header
			...
			switch (e.KeyCode) {
				...
				case Keys.P:
					...
					if (m_textControl.CanPrint) {
						m_textControl.Print(m_fileHandler.DocumentTitle + " - " + ProductName);
					}
					else e.Handled = true;
					break;
			}
		...
		**-----------------------------------------------------------------------------------------------------------*/
		private void OpenFileInNewInstance(string strTarget) {
			...
			if (!File.Exists(strTarget)) {
				Utils.MessageBox.Show(this, string.Format(Resources.MSG_FILE_DOES_NOT_EXIST, strTarget), "Hyperlink", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
