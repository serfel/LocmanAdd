...
namespace TX_Text_Control_Words.FileHandling
{
	public class FileHandler : INotifyPropertyChanged
	{
		...
		public bool HandleUnsavedChanges()
		{
			...
			{
				string text = string.Format(Resources.SAVE_CHANGES_TO, this.DocumentTitle);
				ShowMessageBoxEventArgs showMessageBoxEventArgs = new ShowMessageBoxEventArgs(text, MessageBoxButton.YesNoCancel, MessageBoxIcon.Question);
				this.OnShowMessageBox(showMessageBoxEventArgs);
		...
		{
			FilterException.FilterError reason = exc.Reason;
			if (reason == FilterException.FilterError.InvalidPassword)
			{
		...
		private void HandleMergeBlockConversionException(MergeBlockConversionException exc)
		{
			...
			string text = string.Format(Resources.EXC_MERGE_BLOCK_CONVERSION, arg);
			ShowMessageBoxEventArgs e = new ShowMessageBoxEventArgs(text, MessageBoxButton.OK, MessageBoxIcon.Information);
			this.OnShowMessageBox(e);
		}
